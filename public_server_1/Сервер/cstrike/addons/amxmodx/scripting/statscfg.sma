...
public plugin_init()
{
	register_plugin("Stats Configuration", AMXX_VERSION_STR, "AMXX Dev Team & Wildness Team")
...
public actionCfgMenu(id, key)
{
	switch (key)
	{
		case 7:
		{
			...
			{
				g_modified = false
				client_print_color(id, DontChange, "^04[AMXX] ^01%L", id, "STATS_CONF_SAVED")
			}
			else
				client_print_color(id, print_chat, "^04[AMXX] ^01%L", id, "STATS_CONF_FAILED")
