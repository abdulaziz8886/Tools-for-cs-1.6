...
public actionMenu(id, key)
{
	switch (key)
	{
		case 7:
		{
			...
			{
				g_Modified = false
				client_print_color(id, DontChange, "^04[AMXX] ^01%L", id, "CONF_SAV_SUC")
			}
			else
				client_print_color(id, DontChange, "^04[AMXX] ^01%L", id, "CONF_SAV_FAIL")
...
public plugin_init()
{
	register_plugin("Restrict Weapons", AMXX_VERSION_STR, "AMXX Dev Team & Wildness Team")
