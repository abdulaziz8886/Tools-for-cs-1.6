...
{
	"ДВОЙНОЕ УБИЙСТВО! %s^n%L %d %L (%d %L)", 
	"ПРОСТО МАНЬЯК!!! %s^n%L %d %L (%d %L)", 
	"%s СЕРИЯ УБИЙСТВ!!!^n%L %d %L (%d %L)", 
	"НЕИСТОВЫЙ КИЛЛЕР!!! %s^n%L %d %L (%d hs)", 
	"%s УЖЕ НЕ ОСТАНОВИТЬ!!!^n%L %d %L (%d %L)", 
	"%s ЭТО ЧУДОВИЩЕ!^n%L %d %L (%d %L)", 
	"%s БОЖЕСТВЕНЕН!!!!^n%L %d %L (%d %L)"
}
...
public plugin_init()
{
	register_plugin("CS Misc. Stats", AMXX_VERSION_STR, "AMXX Dev Team & Wildness Team")
...
public client_death(killer, victim, wpnindex, hitplace, TK)
{
	...
	if ((KillingStreak || KillingStreakSound) && !TK)
	{
		...
		if (!selfkill)
		{
			...
			if ((a > -1) && !(a % 2))
			{
				...
				if (KillingStreak)
				{
					set_hudmessage(0, 0, 255, 0.05, 0.50, 2, 0.02, 6.0, 0.01, 0.1, -1)
	...
	if (EnemyRemaining && is_user_connected(victim))
	{
		...
		if (team == CS_TEAM_T || team == CS_TEAM_CT)
		{
			...
			if (pplnum && epplnum)
			{
				...
				set_hudmessage(255, 228, 178, 0.02, 0.85, 2, 0.05, 0.1, 0.02, 3.0, -1)
	...
	if (LastMan)
	{
		...
		if (ctsnum == 1 && tsnum == 1)
		{
			...
			set_hudmessage(125, 249, 255, -1.0, 0.35, 0, 6.0, 6.0, 0.5, 0.15, -1)
		...
		else if (!g_LastAnnounce)
		{
			...
			if (g_LastAnnounce)
			{
				...
				set_hudmessage(125, 249, 255, -1.0, 0.38, 0, 6.0, 6.0, 0.5, 0.15, -1)
	...
	if (wpnindex == CSW_KNIFE && (KnifeKill || KnifeKillSound))
	{
		if (KnifeKill)
		{
			...
			set_hudmessage(247, 84, 225, -1.0, 0.25, 1, 6.0, 6.0, 0.5, 0.15, -1)
	...
	if (headshot && (HeadShotKill || HeadShotKillSound))
	{
		if (HeadShotKill && wpnindex)
		{
			...
			for (new i = 0; i < pnum; i++)
			{
				...
				set_hudmessage(0, 0, 255, -1.0, 0.30, 0, 6.0, 6.0, 0.5, 0.15, -1)
...
public eNewRound()
{
	if (read_data(1) == floatround(get_cvar_float("mp_roundtime") * 60.0,floatround_floor))
	{
		...
		if (RoundCounter)
		{
			set_hudmessage(255, 36, 0, -1.0, 0.30, 0, 6.0, 6.0, 0.5, 0.15, -1)
...
announceEvent(id, message[])
{
	...
	get_user_name(id, name, 31)
	set_hudmessage(255, 140, 105, -1.0, 0.30, 0, 6.0, 6.0, 0.5, 0.15, -1)
