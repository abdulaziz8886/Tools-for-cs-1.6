...
public plugin_init()
{
	register_plugin("Admin Votes", AMXX_VERSION_STR, "Wildness Team")
...
public autoRefuse()
{
	...
	client_print_color(0, DontChange, "^04[AMXX] ^01%L", LANG_PLAYER, "RES_REF")
}
...
public actionResult(id, key)
{
	...
	switch (key)
	{
		case 0:
		{
			...
			client_print_color(0, DontChange, "^04[AMXX] ^01%L", LANG_PLAYER, "RES_ACCEPTED")
		}
		case 1: autoRefuse()
	}
...
public checkVotes()
{
	...
	if (iResult < iRatio)
	{
		...
		for (i = 0; i < pnum; i++)
		{
			...
			if (g_yesNoVote)
				client_print_color(players[i], DontChange, "^04[AMXX] %L", players[i], "VOTING_RES_1", lVotingFailed, g_voteCount[0], g_voteCount[1], iRatio)
			else
				client_print_color(players[i], DontChange, "^04[AMXX] %L", players[i], "VOTING_RES_2", lVotingFailed, iResult, iRatio)
		}
	...
	for (i = 0; i < pnum; i++)
	{
		...
		client_print_color(players[i], DontChange, "^04[AMXX} %L", players[i], "VOTING_RES_3", lVotingSuccess, iResult, iRatio, g_Execute)
	}
...
public voteCount(id, key)
{
	if (get_cvar_num("amx_vote_answers"))
	{
		...
		if (g_yesNoVote)
			client_print_color(0, DontChange, "^04[AMXX] %L", LANG_PLAYER, key ? "VOTED_AGAINST" : "VOTED_FOR", name)
		else
			client_print_color(0, DontChange, "^04[AMXX]%L", LANG_PLAYER, "VOTED_FOR_OPT", name, key + 1)
	}
	++g_voteCount[key]
