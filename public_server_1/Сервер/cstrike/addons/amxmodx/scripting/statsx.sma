...
{
	// Register plugin.
	register_plugin("StatsX", AMXX_VERSION_STR, "AMXX Dev Team & Wilness Team")
...
public cmdStatsMe(id)
{
	if (!SayStatsMe)
	{
		client_print_color(id, DontChange, "^04[StatsX] ^01%L", id, "DISABLED_MSG")
		return PLUGIN_HANDLED
	}
...
public cmdRank(id)
{
	...
	client_print_color(id, DontChange, "%L", id, "YOUR_RANK_IS", iRankPos, iRankMax, izStats[STATS_KILLS], izStats[STATS_HITS], fEff, fAcc)
...
public cmdScore(id)
{
	...
	client_print_color(id, DontChange, "^01%L: ^04%s", id, "GAME_SCORE", g_sScore)
...
public cmdSwitch(id)
{
	...
	client_print_color(id, DontChange, "%L", id, "STATS_ANNOUNCE", lEnDis)
