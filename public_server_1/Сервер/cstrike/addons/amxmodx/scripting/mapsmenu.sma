...
public plugin_init()
{
	register_plugin("Maps Menu", AMXX_VERSION_STR, "AMXX Dev Team & Wildness Team")
...
public checkVotes(id)
{
	...
	if (iResult >= iRatio)
	{
		...
		new tempMap[32];
		ArrayGetString(g_mapName, g_choosed, tempMap, charsmax(tempMap));
		client_print_color(0, DontChange, "^01[^04VoteMap^01] %L ^04%s", LANG_PLAYER, "VOTE_SUCCESS", tempMap);
...
public voteCount(id, key)
{
	...
	if (get_cvar_float("amx_vote_answers"))
	{
		...
		client_print_color(0, DontChange, "%L", LANG_PLAYER, "X_VOTED_FOR", name, key + 1)
	}
