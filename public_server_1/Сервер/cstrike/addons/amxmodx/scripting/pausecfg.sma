...
public plugin_init()
{
	register_plugin("Pause Plugins", AMXX_VERSION_STR, "AMXX Dev Team & Wildness Team")
	...
	switch (key)
	{
		case 6:
		{
			if (file_exists(g_fileToSave))
			{
				...
				client_print_color(id, DontChange, "^04[AMXX] ^01%L", id, "PAUSE_CONF_CLEARED")
			}
			else
				client_print_color(id, DontChange, "^04[AMXX] ^01%L", id, "PAUSE_ALR_CLEARED")
		...
		case 7:
		{
			...
			{
				g_Modified = 0
				client_print_color(id, DontChange, "^04[AMXX] ^01%L", id, "PAUSE_CONF_SAVED")
			}
			else
				client_print_color(id, DontChange, "^04[AMXX] ^01%L", id, "PAUSE_SAVE_FAILED")
		...
		default:
		{
			...
			switch (status[0])
			{
				...
				case 's':
				{
					client_print_color(id, DontChange, "%L", id, "CANT_UNPAUSE_PLUGIN", file);
				}
			}
