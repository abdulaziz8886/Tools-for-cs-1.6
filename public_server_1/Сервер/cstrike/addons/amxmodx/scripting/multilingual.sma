...
public plugin_init()
{
	register_plugin("Multi-Lingual System", AMXX_VERSION_STR, "AMXX Dev Team & Wildness Team")
...
public dispInfo(id)
{
	if (get_cvar_num("amx_client_languages"))
		client_print_color(id, DontChange, "%L", id, "TYPE_LANGMENU")
}
#endif
...
public cmdLangMenu(id, level, cid)
{
	...
	if (!get_cvar_num("amx_client_languages"))
	{
		client_print_color(id, DontChange, "^04[AMXX] ^01%L", LANG_SERVER, "LANG_MENU_DISABLED")
		return PLUGIN_HANDLED
	}
...
public actionMenu(id, key)
{
	...
	if (isAdmin && (key == 2) && !equali(sLang, sLang_old))
	{
		...
		client_print_color(id, DontChange, "%L", pLang, "SET_LANG_SERVER", lName)
	}
	...
	if (!equali(pLang, pLang_old) && ((isAdmin && (key == 2)) || (!isAdmin && (key == 1))))
	{
		...
		client_print_color(id, DontChange, "%L", pLang, "SET_LANG_USER", lName)
	}
